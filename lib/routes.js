...
Router.route("/profile/:username",{
    ...
    waitOn: function(){
        return [subs.subscribe("user.one", this.params.username), subs.subscribe("publication.one.all", this.params.username), subs.subscribe("publication.tagged.one.all", this.params.username)];
    }
});
...
Router.route("/profile/:username/images",{
    ...
    waitOn: function(){
        return [subs.subscribe("user.one", this.params.username), subs.subscribe("publication.one.none", this.params.username), subs.subscribe("image.one.miniature", this.params.username)];
    }
});
...
Router.route('/me', {
    ...
    data: function () {
        return {
            me: Meteor.user(),
            numPublication: Publications.find({"owner.id": Meteor.userId()}).fetch().length
        };
    },
    waitOn: function(){
        return [subs.subscribe("user.me"), subs.subscribe("publication.me.all"), subs.subscribe("publication.tagged.all")];
    },
    name: 'myProfile'
});
...
Router.route('/settings/change-password', function () {
...
Router.route('/settings/privacity', function () {
    this.render('privacity');
}, {
    name: 'privacity',
    before: filters.isLogued
});
