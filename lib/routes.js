...
Router.route('/profile/:_usernameUser', {
    template: 'profileUserTimeline',
    data: function () {
        return {
            user: Meteor.users.findOne({'username':this.params._usernameUser}),
            numPublication: Publications.find().fetch().length
        };
    },
    waitOn: function(){
        var usernameUser = this.params._usernameUser;
        return [Meteor.subscribe("user.user", usernameUser), Meteor.subscribe("publication.user.none", usernameUser)];
    },
    name: 'userProfile'
});
