Template.images_comment.helpers({
    username: function(){
...
Template.images_comment.events({
    ...
    'submit .edit-comment': function(e) {
        ...
        if (valido) {
            Meteor.call('image.editComment', commentId, descriptionTrim, function(err, response){
                if (!err){
    ...
    'submit .remove-comment': function(e) {
        e.preventDefault();
        var commentId = this.id;
        Meteor.call('image.removeComment', commentId, function(err, response){
            if (!err){
    ...
    'click #like': function (e) {
        e.preventDefault();
        var commentId = this.id;
        Meteor.call('image.likeComment', commentId, function(err, response){
            if(err){
                console.log(err);
            }
        });
    ...
    'click #dislike': function (e) {
        e.preventDefault();
        var commentId = this.id;
        Meteor.call('image.dislikeComment', commentId, function(err, response){
            if(err){
                console.log(err);
            }
        });
    ...
    'click #i-like': function (e) {
        ...
        if (_.contains(this.playersLike, Meteor.userId())){
            Meteor.call('image.removeLikeComment', commentId, function(err, response){
                if(err){
                    console.log(err);
                }
            });
    ...
    'click #i-dislike': function (e) {
        ...
        if (_.contains(this.playersDislike, Meteor.userId())){
            Meteor.call('image.removeDislikeComment', commentId, function(err, response){
                if(err){
                    console.log(err);
                }
            });
